...
const exampleId = ref<StructuralExample['id']>(DEFAULT_EXAMPLE_BY_MODULE[props.moduleId])
...
watch(() => props.moduleId, (moduleId) => {
  if (request.value.analysis === moduleId) return
  selectExample(DEFAULT_EXAMPLE_BY_MODULE[moduleId])
})
...
<template>
  <StructuralWorkspace
    :module-id="request.analysis"
    :show-module-selector="false"
    compact
    @module-change="selectModule"
  >
    <template #workspace>
