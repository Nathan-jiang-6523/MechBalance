...
**当前门禁**：P3 确认表、验收算例表尚未由用户签收。本轮只能创建门禁文档，不得开发求解器或 UI。
...
### [ ] P3-B01：盘点 P1 可复用公共能力

**描述**：只检查单位、材料、数值、校验、警告、公式组件、结果契约的公开接口。

**验收标准**：形成复用映射；不复制单位表；列出确需扩展的共享文件。

**文件**：`mechanics-toolbox/src/core/units/`、`materials/`、`validation/`、`contracts/`，只读盘点记录写入任务备注。
...
### [ ] P3-B07：建立 P3 fixture JSON schema
...
### [ ] P3-RP10：登记第二载荷公式（条件性）

**描述**：首工况通过后，登记一个已确认载荷的独立公式。
...
### [ ] P3-RP10B：实现第二载荷内核（条件性）

**描述**：实现 RP10 已登记载荷，不扩展其他边界/载荷组合。
...
### [ ] P3-H02：接入主导航（独立 integration 提交）
...
- [ ] FluxUI 组件：不适用；项目不使用 Laravel/Livewire/FluxUI。
- [ ] 所有命令不能有后台进程；绝对不加 `&`。
- [ ] 不写启动服务器命令；默认开发服务器已运行。
- [ ] 必须适配桌面与移动端。
- [ ] 所有计算器必须显式选择边界条件。
- [ ] 表单非法时旧结果失效，禁止显示成功状态。
- [ ] 图片优先使用自绘 SVG；如需外部占位图，只用 Unsplash 或 `https://picsum.photos/`，不用 Pexels。
- [ ] 公式统一使用 `MathFormula.vue`；长公式允许横向滚动。
- [ ] 执行项目 Playwright：`npm run test:e2e`。
- [ ] 如仓库提供通用截图脚本，执行：`./qa-playwright-capture.sh http://localhost:8000 public/qa-screenshots`；当前不得为满足命令擅自启动服务器或新增脚本。
- [ ] 运行 `npm run typecheck`、`npm run test`、`npm run build`。
- [ ] 不读取用户本地参考书/PDF，不读取用户排除目录。
- [ ] 不修改 P2/P4 专属目录，不创建 remote，不推送。
- [ ] 不审批自己的 diff；由主 agent/用户独立审查。
...
**时间线预期**：当前不承诺。完成 P3-A05、A06、A07 后，按实际保留的 30–60 分钟任务数估算；日历计划必须包含公式审核、用户真值反馈、独立对照和 2–3 轮修改。
